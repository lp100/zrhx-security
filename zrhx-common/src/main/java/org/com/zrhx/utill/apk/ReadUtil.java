...
public final class ReadUtil {
    ...
    private  final static Logger logger = Logger.getLogger(ReadUtil.class);
    /**
     * 读取apk
     * @param apkUrl
     * @return
     */
    public static ApkInfo readAPK(File apkUrl){
        ZipFile zipFile = null;
//        Map<String,Object> map = new HashMap<String, Object>(1);
         ApkInfo map = new ApkInfo();
        try {
                            ...
                            if(null != name && name.toLowerCase().equals("manifest")){
                                for (int i = 0; i != parser.getAttributeCount(); i++) {
                                    if ("versionName".equals(parser.getAttributeName(i))) {
                                        ...
                                        map.setVersionName(versionName);
                                    } else if ("package".equals(parser.getAttributeName(i))) {
                                        String packageName = getAttributeValue(parser, i);
                                        if(null == packageName){
                                            packageName = "";
                                        }
                                        if(!"com.zrhx.airporttrain".equals(packageName)){
                                        	 map.setCode(Constants.ERROR_CODE);
                                        	 map.setError("请上传正确的文件！");
                                        	 break;
                                        }
                                        map.setPackageName(packageName);
                                    ...
                                    map.setCode(Constants.SUCCESS_CODE);
                                }
                                break;
                            }
                        }
    ...
    public static void main(String[] args) {
        ...
        String apkUrl = "D:\\Tomcat 7.0\\webapps\\airport-1.2.4.apk";
        ApkInfo mapApk = ReadUtil.readAPK(new File(apkUrl));
